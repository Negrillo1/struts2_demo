  ...
  <body>
		<table class="easyui-datagrid"
       	...
       	第${pb.pc }页/共${pb.tp }页
        	...
        	<a href="<%=path %>/student/Student_queryAll">首页</a>&nbsp;
        	<c:if test="${pb.pc > 1 }">
        	<a href="<%=path %>/student/Student_queryAll?pc=${pb.pc-1 }">上一页</a>&nbsp;
        	</c:if>
        	...
        	<c:forEach var="i" begin="${begin }" end="${end }">
        		<a href="<%=path %>/student/Student_queryAll?pc=${i }">${i }</a>&nbsp;&nbsp;
        	</c:forEach>
        	<c:if test="${pb.pc < pb.tp }">
        	<a href="<%=path %>/student/Student_queryAll?pc=${pb.pc+1 }">下一页</a>&nbsp;
        	</c:if>
        	<a href="<%=path %>/student/Student_queryAll?pc=${pb.tp}">尾页</a>&nbsp;
			<a href="<%=path %>/student/Student_exportExcel">导出当前页</a>&nbsp;
			<a href="<%=path %>/student/Student_exportExcelAll">导出全部信息</a>&nbsp;
